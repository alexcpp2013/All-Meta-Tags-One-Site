...
namespace MetaTags
{
    public partial class Form1 : Form
    {
                ...
                if (elems != null)
                {
                    foreach (HtmlElement elem in elems)
                    {
                        ...
                        if (nameStr != null && nameStr.Length != 0)
                        {
                            ...
                            AddAttrbuteToList(new Tuple<string, string>(nameStr, contentStr));
                        }
                    }
                }
            }
        ...
        private void AddAttrbuteToList(Tuple<string, string> cur)
        {
            /*if (comboBox1.Items.IndexOf(cur.Item1.ToLower()) < 0)
                return;*/
            bool flag = false;
            for(int i = 0; i < comboBox1.Items.Count; ++i)
            {
                if(comboBox1.Items[i].ToString().ToLower() == cur.Item1.ToLower())
                {
                    flag = true;
                    return;
                }
            ...
            if(!flag)
                comboBox1.Items.Add(cur.Item1.ToLower());
        }
